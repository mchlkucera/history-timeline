/**
 * slabs.js — turn flat lon/lat polygons into *slabs*: real prisms with
...
export class SlabBuilder {
  ...
  /**
   * @param {{o:number[],h:number[][]}} poly  outer ring + hole rings, flat [lon,lat,...]
   * @param {number} zBot @param {number} zTop
   * @param {number} sx @param {number} sy  degrees -> world units (linear projection)
   */
  addPolygon(poly, zBot, zTop, sx, sy) {
    const { o, h } = poly;
    // ---- flatten for earcut -------------------------------------------------
    const flat = o.slice();
    const holeIdx = [];
