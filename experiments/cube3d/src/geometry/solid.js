/**
 * solid.js — build a genuine 3-D volume for one sovereign.
 *
...
function edt2(mask, nx, ny, want, out, scratch) {
...
function rasterize(rings, nx, ny, x0, y0, cell, mask) {
  mask.fill(0);
  const xs = [];
  for (let j = 0; j < ny; j++) {
 ...
 */
export function surfaceNets(data, nx, ny, nz) {
...
function relax(pos, idx, iters, lambda) {
  if (iters <= 0) return;
...
function gradientNormals(pos, data, nx, ny, nz) {
...
/**
 * @param {object} o
 * @param {Array<Array<number[]>|null>} o.presence  per snapshot index: array of flat rings (deg) or null
 * @param {number[]} o.zc      slab centre z per snapshot index (world units)
 * @param {number}  o.half     slab half-thickness (world units)
 * @param {number}  o.res      target cells on the long axis
 * @param {number}  o.sub      sub-layers generated per inter-snapshot gap
 * @param {number}  o.band     |sdf| clamp, in degrees
 * @param {(lon:number)=>number} o.lonToX
 * @param {(lat:number)=>number} o.latToY
 */
export function buildSovereignVolume(o) {
  ...
  // ---- z layer plan -------------------------------------------------------
  const zOfLayer = [];
  const srcLayer = [];   // snapshot index whose field this layer uses, or -1 == empty
  const mixLayer = [];   // [iA, iB, t] for blended layers
  ...
  for (let i = first; i <= last; i++) {
    ...
    for (let k = 0; k < N; k++) {
      let s = (Math.sqrt(dOut[k]) - Math.sqrt(dIn[k])) * cell;
  ...
  for (let i = 0; i < positions.length; i += 3) {
    ...
    let gx = normals[i] / cell, gy = normals[i + 1] / cell, gz = normals[i + 2] / Math.max(1e-4, dz);
 ...
 */
export function buildSovereignVolumes(o) {
  ...
  for (const g of groups.values()) {
    ...
    clusters.push({ g, span, w: x1 - x0 + 2 * band, h: y1 - y0 + 2 * band });
  }
  ...
  const voxels = (m) => clusters.reduce((s, k) => s + Math.ceil(k.w / (k.cell * m)) * Math.ceil(k.h / (k.cell * m)) * nzGuess, 0);
  let mult = 1, guard = 0;
...
function countLayers(presence, sub) {
  let first = -1, last = -1;
